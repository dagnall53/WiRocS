...
//#ifdef ESP32
  ...
  // moved to oled_initiate  SSD1306  display(0x3c, OLED_SDA, OLED_SCL);
...
void Status(){
  ...
  while ((WiFi.status() != WL_CONNECTED) && (millis()<= LoopTimer)) {
              digitalWrite (NodeMCUPinD[SignalLed] , SignalON) ; 
              delay(150);
              digitalWrite (NodeMCUPinD[SignalLed] , SignalOFF) ;
              delay(150);
              Serial.print(".");}
  ...
  OLED_5_line_display(1,wifiSSID.c_str(),"","","Connecting to MQQT","");
  OLED_5_line_display(2,"Connected to",wifiSSID.c_str(),"","","");
...
//----------------------Setup from eeprom
  ...
  EEPROM.begin(Serial_EEPROM_Starts+EEPROM_Serial_store_Size);
      ...
      if ((EEPROM.read(255) == 0xFF) && (EEPROM.read(256) == 0xFF)){SetDefaultEEPROM();} //good chance the eeprom is empty, or we are forcing this probably this is a first run. Can also set via CV[8]=8
  #endif
...
// This code to help me whilst I sort some data overflows that are corrupting RocnodeID
//  if (EEPROMRocNodeID!= RocNodeID){Serial.print("RocnodeID :");Serial.print(RocNodeID);Serial.print(" eeprom:");Serial.print(EEPROMRocNodeID);Serial.println(" Forcing ID = 50");RocNodeID=50;WriteWiFiSettings();} // force for tests
// will remove it later
 ...
 if (!MQTT_Connected()) {    //if NOT connected, try to connect, but if we are connected, the you can do the main loop stuff.
    ...
    connects = connects + 1;
    reconnect();
    if ((connects>=2)&& (mosquitto[3]!=BrokerAddr))
       { // Save the address for next time!
        BrokerAddr=mosquitto[3];
        Serial.println(F("Saving new Broker address"));
        WriteWiFiSettings();
      }
  }else{
...
//Sign of life flash and clock display
  if (LoopTimer >= lastsec ) {//sign of life flash 
    digitalWrite (NodeMCUPinD[SignalLed] , SignalON) ; ///turn On
    #ifdef _ESP32_HallTest
      lastsec = lastsec +5000;  secs = secs + (5*divider);  Hall=hallRead();
      Serial.print("Hall<");Serial.print(Hall);Serial.print("> Temp:");// Serial.print((temprature_sens_read() - 32) / 1.8);Serial.println(" C");
     #else
      lastsec = lastsec +1000;secs = secs + divider; TimeGears();
      OLED_Status();
              #ifdef _SERIAL_Audio_DEBUG
                //Serial.print(ChuffDebugState);
                  Serial.print(Wavs_Per_Revolution);
              #else
                 // Serial.print(".");
              #endif
    #endif
